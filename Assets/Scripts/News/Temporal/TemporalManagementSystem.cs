...
namespace ElectionEmpire.News.Temporal
{
    ...
    public class NewsCycleManager
    {
        ...
        public NewsCycleManager(IGameTimeProvider timeProvider = null, TemporalConfig config = null)
        {
        ...
        {
            // Check if events are from same source news (exact match)
            if (!string.IsNullOrEmpty(a.SourceNewsId) && !string.IsNullOrEmpty(b.SourceNewsId))
            {
                if (a.SourceNewsId == b.SourceNewsId)
                {
                    return true;
                }
                
                // Check if they share a common prefix (same news source)
                var aPrefix = a.SourceNewsId.Split('_')[0];
                var bPrefix = b.SourceNewsId.Split('_')[0];
            ...
            // Check temporal proximity (events within 24 hours are more likely similar)
            var timeDiff = Mathf.Abs((float)(a.OriginalPublishTime - b.OriginalPublishTime).TotalHours);
            if (timeDiff < 24f)
            {
                // Additional similarity checks could go here
                // For now, temporal proximity alone isn't enough
            }
            
            // Could add category matching, keyword overlap, etc. here
            // For now, only exact source matches are considered similar
            return false;
        }
        ...
        /// <summary>
        /// Legacy method for compatibility with old NewsCycleManager
        /// </summary>
        public void CreateCycle(ProcessedNews news)
        {
            // Convert ProcessedNews to temporal state
            string eventId = news.OriginalArticle?.Title?.GetHashCode().ToString() ?? Guid.NewGuid().ToString();
            RegisterEvent(eventId, eventId, news.ProcessedDate);
        }
        ...
        public void UpdateCycles(float deltaTime)
        {
            ...
            if ((DateTime.UtcNow - _lastUpdateTime).TotalSeconds >= _config.CycleUpdateIntervalSeconds)
            {
                Update();
            }
        }
