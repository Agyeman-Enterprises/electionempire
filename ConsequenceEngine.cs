...
namespace ElectionEmpire.News.Consequences
{
    ...
    public class ConsequenceCalculator
    {
        ...
        private List<ConsequenceEffect> ExtractBaseEffects(ResponseOption response)
        {
            ...
            // Alignment effects
            if (response.AlignmentShift != null)
            {
                if (response.AlignmentShift.TryGetValue("law_chaos", out float lawChaos))
                {
                    effects.Add(new ConsequenceEffect
                    {
                        EffectId = Guid.NewGuid().ToString(),
                        ...
                        BaseMagnitude = lawChaos
                    });
                }
                
                if (response.AlignmentShift.TryGetValue("good_evil", out float goodEvil))
                {
                    effects.Add(new ConsequenceEffect
                    {
                        EffectId = Guid.NewGuid().ToString(),
            ...
            ConsequenceContext context)
        {
            foreach (var effect in effects)
            {
                ...
                {
                    EventUrgency.Critical => 1.5f,
                    EventUrgency.High => 1.25f,
                    EventUrgency.Medium => 1.0f,
                    EventUrgency.Low => 0.75f,
                    _ => 1.0f
                };
                ...
                // Media attention scaling
                float mediaMultiplier = Mathf.Lerp(0.7f, 1.3f, gameEvent.MediaCoverage);
                scaling *= mediaMultiplier;
                effect.Modifiers["media_attention"] = mediaMultiplier;
        ...
        private float CalculateConsistencyModifier(ConsequenceEffect effect, ConsequenceContext context)
        {
            ...
            return 1.0f; // Placeholder - would analyze stance history
        }
            ...
            out float threshold)
        {
            ...
            threshold = response.BaseSuccessChance;
            ...
            ConsequenceContext context)
        {
            ...
            if (gameEvent.EventType == GameEventType.Crisis)
            {
                if (isSuccess)
                {
        ...
        private void DetermineEventChains(NewsGameEvent gameEvent, ResponseOption response, ResponseResult result)
        {
            ...
            // Placeholder for event chaining system
        }
    ...
    public enum GameEventType { PolicyPressure, Crisis, Opportunity, Scandal }
